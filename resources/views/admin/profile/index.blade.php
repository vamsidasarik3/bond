...
<div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div class="relative group shrink-0">
                <img id="avatarPreview" src="{{ $user->avatar_url }}" alt="{{ $user->name }}" 
                     class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl object-cover ring-4 ring-brand-500/20 shadow-md">
                
                <label for="avatarInput" title="Click to change photo"
                       class="absolute inset-0 bg-slate-950/60 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white cursor-pointer text-xs font-semibold">
            ...
            <div class="text-center sm:text-left flex-1 min-w-0">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <span class="inline-flex items-center px-3 py-1 text-xs font-bold rounded-full bg-brand-50 text-brand-700 border border-brand-200 uppercase tracking-wider self-center sm:self-auto">
                        <i class="fa-solid fa-shield-halved mr-1.5 text-brand-600"></i> Administrator
                    </span>
                </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 shadow-xs">
        <div class="border-b border-slate-100 pb-4 mb-6">
            <h3 class="text-base font-extrabold text-slate-900 tracking-tight">Profile Information</h3>
        ...
        <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500">
                        Upload a new photo (JPEG, PNG, JPG, or WEBP up to 2MB). Previous photo is automatically replaced.
                    </p>
                    @error('avatar')
                ...
                <div class="flex items-center gap-2 shrink-0">
                    <label for="avatarInput" class="px-4 py-2 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 cursor-pointer transition-colors shadow-2xs">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" required
                            ...
                            class="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 font-semibold focus:outline-none focus:bg-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                    </div>
                    @error('name') 
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 mt-1">Unique login handle (alphanumeric, dashes, underscores)</p>
                    @error('username') 
                        <p class="text-[11px] text-rose-600 mt-1 font-medium">{{ $message }}</p> 
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="password" id="password" name="password" autocomplete="new-password"
                                ...
                                class="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                        </div>
                        @error('password') 
                            <p class="text-[11px] text-rose-600 mt-1 font-medium">{{ $message }}</p> 
                        @enderror
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                ...
                <button type="submit" class="px-7 py-2.5 bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold rounded-xl shadow-md shadow-brand-600/25 transition-all flex items-center gap-2">
    ...
    function handleAvatarChange(input) {
        if (input.files && input.files[0]) {
            ...
            if (file.size > 2097152) {
                alert('The selected image exceeds 2MB limit. Please choose a smaller photo.');
                input.value = '';
                return;
            }
